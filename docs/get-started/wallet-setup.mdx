...
<Warning>
Your private signing keys are sent to the bot's server RAM so the worker can sign bids automatically. The server has full signing authority over your bidding addresses while the bot is running — treat OrdsBot as a trusted signing agent. Keys are never written to disk or the database, and are lost on server restart. You will see a "Waiting for wallet keys" warning on your tasks when re-derivation is needed.
</Warning>
